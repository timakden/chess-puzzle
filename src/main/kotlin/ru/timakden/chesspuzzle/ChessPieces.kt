...
class Bishop(row: Int, column: Int) : ChessPiece(row, column) {
    ...
    override fun canAttackAnotherPiece(anotherPiece: ChessPiece): Boolean {
        return abs(row - anotherPiece.row) == abs(column - anotherPiece.column)
    }
}
...
class King(row: Int, column: Int) : ChessPiece(row, column) {
    ...
    override fun canAttackAnotherPiece(anotherPiece: ChessPiece): Boolean {
        return abs(row - anotherPiece.row) == 1 && abs(column - anotherPiece.column) == 1 ||
...
class Knight(row: Int, column: Int) : ChessPiece(row, column) {
    ...
    override fun canAttackAnotherPiece(anotherPiece: ChessPiece): Boolean {
        return abs(row - anotherPiece.row) == 2 && abs(column - anotherPiece.column) == 1 ||
...
class ChessPieceFactory {
    fun getPiece(pieceType: String, row: Int, column: Int): ChessPiece {
        return when (pieceType) {
            ...
            "Q" -> Queen(row, column)
            "B" -> Bishop(row, column)
            else -> throw IllegalArgumentException("Неизвестный тип фигуры: $pieceType")
        }
    }
}
